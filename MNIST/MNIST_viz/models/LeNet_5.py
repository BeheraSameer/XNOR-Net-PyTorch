    ...
    def forward(self, x):
        ...
        if not self.Linear:
            ...
            for i in range(20):
                ...
                plt.axis('off')
            plt.savefig('pre_activations_{}.png'.format(time.time()))
            x = self.conv(x)
            K = self.k_conv(A)
            x = x * K 
        else:
...
class LeNet_5(nn.Module):
    ...
    def forward(self, x):
        ...
        for i in range(1):
            ...
            plt.imshow(x[0][i], cmap='gray')
            plt.axis('off')
        plt.savefig('activations_orig_{}.png'.format(time.time()))
        time.sleep(1)
        x = self.conv1(x)
        x = self.bn_conv1(x)
        x = self.relu_conv1(x)
        ...
        for i in range(20):
            ...
            plt.imshow(x[0][i], cmap='gray')
            plt.axis('off')
        plt.savefig('activations_conv1_{}.png'.format(time.time()))
        time.sleep(1)
        x = self.bin_conv2(x)
        x = self.pool2(x)
        fig = plt.figure()
        for i in range(50):
            ...
            plt.imshow(x[0][i], cmap='gray')
            plt.axis('off')
        plt.savefig('activations_unit_2_{}.png'.format(time.time()))
        time.sleep(1)
